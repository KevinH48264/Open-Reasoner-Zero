    ...
    async def make_experience(self, all_inputs: Union[Tuple[str, dict], List[Tuple[str, dict]]], **generate_kwargs):
        ...
        if self.self_play:
            ...
            def extract_answer_input(query_item):
                ...
                assistant_suffix = " Show your work in <think> </think> tags. And return the final equation in <answer> </answer> tags, for example <answer> (1 + 2) / 3 </answer>. Think step by step inside <think> tags.\nAssistant: <think>" # Give example final answer to help policy model understand output better
                ...
                default_valid_answer_input = (
                    default_full_prompt,
                    {'target': 6, 'nums': [1, 2, 3], 'valid_prompt': False, 'answer': 6}
                )
                ...
                try:
                    ...
                    if not isinstance(answer_input, dict) or not all(k in answer_input for k in ('prompt', 'target', 'nums')):
                    ...
                    answer_input['target'] = int(answer_input.get('target'))
        ...
        if self.self_play:
            ...
            all_targets = [d["target"] for d in query_output_extras if "target" in d]
            ...
            logger.debug("\n\n[INSPECT QUERY TRAINING DATA AND POLICY TRAINING DATA]")
            ...
            for idx in range(num_inspect):
                ...
                logger.debug(f"\nQuery {idx+1} Query Text: {str(query_text)}")
                logger.debug(f"Query {idx+1} Extras: {str(query_output_extras[idx])}")
                logger.debug(f"Query {idx+1} Computed Reward (last token): {query_reward_val}")

                # For the policy branch: each query was repeated R times.
                logger.debug(f"Policy Completions for Query {idx+1}:")
                for j in range(R):
                    ...
                    logger.debug(f"  Policy Completion {j+1}: {comp_text}")
                    logger.debug(f"    Generate VLLM Raw Output: {outputs_dicts[policy_index]}")
                    logger.debug(f"    Extras: {all_extras[policy_index]}")
                    logger.debug(f"    Computed Policy Reward (last token): {policy_reward_val}")
