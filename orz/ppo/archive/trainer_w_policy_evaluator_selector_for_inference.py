...
This is the problem:
Given the Policy Assistant Goal, Game History of previous Query Assistant Answer, Average Success, and Average Response Length, and Policy Assistant Answer and Reward turns, Task Generation Success Criteria, and Example Final Answer Format, generate a task that fulfills the task generation success criteria. 
...
3) difficult yet tractable - the task should be challenging for the Policy Assistant to complete, without being completely intractable. A Query Average Success score that is not 0.0 (too hard or impossible) nor 1.0 (too easy) is ideal.
...
5) logically correct - The 'solution' and 'target' to the 'prompt' must be a full and complete proof and answer to the math problem, such that if the Policy Assistant submitted the provided solution and target answer word-for-word without any modifications, the solution and target would be considered correct reasoning and the correct final answer to the math problem.

6) educational - The task should encourage deeper and longer thinking from the Policy Assistant (the higher the policy assistant response length, the better), often with a streamlined math problem and a solution that - once discovered - often feels natural and almost inevitable. Beyond testing computational skills or rote methods, the task should introduce or reinforce a valuable technique, pattern, or idea that the Policy Assistant can apply in other contexts - making the problem not just a challenge, but also a learning opportunity.

7) formatted - The task and final answer must be a dictionary with 'prompt', 'target', and 'solution' keys. 
...
{\n    'prompt': '<insert math problem as string here>',\n    'target': <insert target expected answer as int or float here>,\n    'solution': '<insert problem solution as string here>'\n}
                    ...
                    for turn, pair_index in enumerate(range(start_pair, num_pairs), start=1):
                        ...
                        prompt_w_history += (
                            f"\n\n**Turn {turn}**\n"
                            f"Query Assistant Response: {query_final_answer}\n\n"
        ...
        # # --- NEW: VERIFIED SOLUTION EXTRACTION FOR POLICY EXPERIMENT --- (Result: this is not necessary. Numerical verification = check signal from environment - aka proof. Also current model is not able to do this well. It'll say the answers are the same in the reasoning, and then return 'No' for if they're the same.)
        ...
        # # If the experiment type is "policy", we need to get a verified_solution for each prompt batch.
        # responses = [output["response"] for output in outputs_dicts]
        # final_answers = [output["final_answer"] for output in outputs_dicts]
        # prompts = all_prompts
        # verified_solution = [""] * len(responses)  # now based on responses instead of final_answers
        # extraction_valid_count = 0
        # none_selected_count = 0
        # total_groups = 0

        # # Initialize lists to collect evaluator prompts, responses, and number of valid options.
        # evaluator_extract_prompts = []
        # evaluator_extract_responses = []
        # evaluator_extract_num_valid_options = []

        # from collections import defaultdict
        # # Group indices by prompt (assumes that all rollouts sharing the same prompt are identical)
        # group_indices = defaultdict(list)
        # for i, prompt in enumerate(prompts):
        #     group_indices[prompt].append(i)
        # letters = "ABCDEFGHIJKLMNOPQRSTUVWXYZ"

        # # For each prompt group, compile valid responses and create a multiple-choice evaluator prompt.
        # for prompt_text, indices in group_indices.items():
        #     total_groups += 1
        #     # Build valid options from responses that have non-empty responses and final answers.
        #     valid_options = ["<think>" + responses[i] for i in indices if responses[i].strip() != "" and final_answers[i].strip() != ""]

        #     # Update format: convert the <think> ... </think>\n\n<answer> ... </answer> to "Reasoning: ... Answer: ..."
        #     regex = r"^<think>([^<]*(?:<(?!/?think>)[^<]*)*)<\/think>\n\n<answer>([\s\S]*)<\/answer>$"
        #     for i, option in enumerate(valid_options):
        #         match = re.search(regex, option, re.DOTALL)
        #         if match is not None and len(match.groups()) == 2:
        #             reasoning, answer = match.group(1).strip(), match.group(2).strip()
        #             valid_options[i] = f"Reasoning: {reasoning}\n\nAnswer: {answer}"
        #         else:
        #             # If regex fails, leave the option unchanged.
        #             valid_options[i] = option

        #     # Record the number of valid options for this group.
        #     num_options = len(valid_options)
        #     evaluator_extract_num_valid_options.append(num_options)

        #     # Build the options string for the evaluator prompt.
        #     options_str = ""
        #     for j, option in enumerate(valid_options):
        #         options_str += f"{letters[j]}: {option}\n\n"
        #     options_str += f"{letters[len(valid_options)]}: None of the above are an easily verifiably correct answer to the problem."

        #     # Optionally clean prompt_text to remove system instructions.
        #     system_prefix = (
        ...
        #         "i.e. your response format should be: <think> reasoning process here </think>\n\n<answer> answer here </answer>. "
        ...
        #         "for example <answer> answer here </answer>. Think step by step inside <think> tags.\nAssistant: <think>"
        #     )
        #     if "This is the problem:\n" in prompt_text:
        #         prompt_text = prompt_text.split("This is the problem:\n", 1)[1].strip()
        #     if " Show your work in <think> </think> tags." in prompt_text:
        #         prompt_text = prompt_text.split(" Show your work in <think> </think> tags.", 1)[0].strip()
            
        #     # Build the evaluator prompt.
        #     evaluator_prompt = (
        #         f"{prompt_text}\n\n"
        #         "This is a single-answer multiple choice question. Do not re-solve the problem yourself. "
        #         "Instead, simply compare the following options and select the one that is most easily verifiably correct:\n\n"
        #         f"{options_str}\n\n"
        #         f"Provide your final choice as a single letter enclosed in <answer> tags. For example, <answer>{letters[0]}</answer>.\n"
        #         "Please also include your reasoning inside <think> tags."
        #     )
        #     evaluator_prompt = system_prefix + evaluator_prompt + assistant_suffix

        #     # Save the evaluator prompt for later use.
        #     evaluator_extract_prompts.append(evaluator_prompt)
        #     evaluator_responses, _ = await gen_func(
        #         prompts=[evaluator_prompt],
        #         sampling_params=sampling_params,
        #         use_tqdm=False,
        #         truncate_prompt=True
        #     )
        #     evaluator_extract_responses.append(evaluator_responses[0])

        #     evaluator_text = "<think>" + evaluator_responses[0]

        #     # Log the evaluator prompt and response.
        #     logger.info(f"[STEP {self.global_step}] [Evaluator Extract Verified Solution]\n--- Prompt: {evaluator_prompt}\n\n--- Response: {evaluator_text}")
        #     self.writer.add_text("evaluator_extract_verified_solution", f"Prompt: {evaluator_prompt}\nResponse: {evaluator_text}", self.global_step)

        #     # Use regex to extract the chosen letter.
        #     regex_match = re.search(regex, evaluator_text, re.DOTALL)
        #     if regex_match is None or len(regex_match.groups()) != 2 or not evaluator_text.endswith("</answer>"):
        #         continue
        #     chosen_letter = regex_match.group(2).strip()
        #     letter_index = letters.find(chosen_letter)
        #     if letter_index == -1:
        #         logger.info(f"[STEP {self.global_step}] [Evaluator Extract] Invalid letter: {chosen_letter}")
        #         continue
        #     extraction_valid_count += 1
        #     if letter_index < len(valid_options):
        #         chosen_option_text = valid_options[letter_index]
        #     else:
        #         chosen_option_text = ""
        #         none_selected_count += 1
        #     for idx in indices:
        #         verified_solution[idx] = chosen_option_text
        #     logger.info(f"[STEP {self.global_step}] [Evaluator Extract] Verified solution: {chosen_option_text}\n")
        # # Log Verified Solution Evaluator Extraction statistics.
        # if total_groups > 0:
        #     self.writer.add_scalar("evaluator_extract/format_valid_ratio", extraction_valid_count / total_groups, self.global_step)
        #     self.writer.add_scalar("evaluator_extract/none_are_correct_ratio", none_selected_count / total_groups, self.global_step) # too difficult of a query maybe
        #     self.writer.add_scalar("evaluator_extract/extracted_verified_solution", (extraction_valid_count - none_selected_count) / total_groups, self.global_step) # valid AND at least 1 rollout was able to be correct


        # # --- REWARD FUNCTION: CHECK AGAINST VERIFIED SOLUTION ---
        # # Now use an asynchronous LM evaluator call to check if the proposed response is verifiably correct.
        # async def check_verified_solution(prompt, response, verified_solution):
        #     '''
        #     Returns a tuple of (is_correct, format_valid, yes_flag, evaluator_prompt, evaluator_text)
        #     '''

        #     # If no verified solution is provided, mark as incorrect.
        #     if not verified_solution:
        #         return (False, 0, 0, None, None)

        #     # Strip system prefix and assistant suffix from the prompt.
        #     if "This is the problem:\n" in prompt:
        #         prompt_clean = prompt.split("This is the problem:\n", 1)[1].strip()
        #     else:
        #         prompt_clean = prompt
        #     if " Show your work in <think> </think> tags." in prompt_clean:
        #         prompt_clean = prompt_clean.split(" Show your work in <think> </think> tags.", 1)[0].strip()
        ...
        #         "for example <answer> answer here </answer>. Think step by step inside <think> tags.\nAssistant: <think>"
        #     )
        #     # Ensure the response starts with <think>
        ...
        #     if match is None or len(match.groups()) != 2 or not response.endswith("</answer>"):
        #         return (False, 0, 0, None, None)
        #     reasoning, answer = match.group(1).strip(), match.group(2).strip()
        ...
        #         system_prefix +
        #         prompt_clean + "\n\n" +
        #         "Proposed response: " + formatted_response + "\n\n" +
        #         "Verified solution: " + verified_solution + "\n\n" +
        #         "This is a single-answer multiple choice Yes or No question. Do not re-solve the problem yourself. Instead, simply compare the proposed response and verified solution and verify if the proposed response is easily verifiably correct based on the verified solution:\n\n" +
        #         "Yes: The proposed response is correct.\n\n" +
        #         "No: The proposed response is NOT correct.\n\n" +
        #         "Provide your final choice as Yes or No enclosed in <answer> tags. For example, <answer>Yes</answer>.\nPlease also include your reasoning inside <think> tags." +
        #         assistant_suffix
        #     )
        ...
        #         stop=self.cfg.stop,
        #     )
        #     async with Timer("Evaluator Check for Verified Solution"):
        #         evaluator_responses, _ = await gen_func(
        #             prompts=[evaluator_prompt],
        ...
        #     logger.info(f"[STEP {self.global_step}] [Evaluator Check]\nPrompt: {evaluator_prompt}\nResponse: {evaluator_text}")
        #     self.writer.add_text("evaluator_check_against_verified_solution",
        #                         f"Prompt: {evaluator_prompt}\nResponse: {evaluator_text}",
        #                         self.global_step)

        #     m = re.search(regex, evaluator_text, re.DOTALL)
        #     if m:
        #         format_valid = 1
        #         extracted_answer = m.group(2).strip()
        #         if extracted_answer.lower() == "yes":
        #             yes_flag = 1
        #             return (True, format_valid, yes_flag, evaluator_prompt, evaluator_text)
        #         elif extracted_answer.lower() == "no":
        #             yes_flag = 0
        #             return (False, format_valid, yes_flag, evaluator_prompt, evaluator_text)
        #         else:
        #             yes_flag = 0
        #             return (False, 0, 0, evaluator_prompt, evaluator_text)
        #     return (False, 0, 0, evaluator_prompt, evaluator_text)

        # # Create asynchronous tasks for each rollout.
        # check_tasks = []
        # for prompt, response, vs in zip(prompts, responses, verified_solution):
        #     check_tasks.append(check_verified_solution(prompt, response, vs))
        # check_results = await asyncio.gather(*check_tasks)

        # # Separate the aggregated results.
        # bool_results = [r[0] for r in check_results]
        # avg_format_valid = sum(r[1] for r in check_results) / len(check_results) if check_results else 0
        # avg_yes = sum(r[2] for r in check_results) / len(check_results) if check_results else 0
        # evaluator_check_prompts = []
        # evaluator_check_responses = [] 
        # for r in check_results:
        #     if r[3] is not None and r[4] is not None:
        #         evaluator_check_prompts.append(r[3])
        #         evaluator_check_responses.append(r[4][len("<think>"):].lstrip() if r[4].startswith("<think>") else r[4])

        # self.writer.add_scalar("evaluator_check/avg_format_valid", avg_format_valid, self.global_step)
        # self.writer.add_scalar("evaluator_check/avg_verifiably_correct", avg_yes, self.global_step)
        # self.writer.add_scalar("evaluator_check/str_match_iscorrect_avg", np.mean(bool_results), self.global_step)

        # for i, (response, extras) in enumerate(zip(outputs, all_extras)):
        #     response["iscorrect"] = bool_results[i] and response["iscorrect"] # most strict policy check - if numerical check AND LM evaluator checks pass. # empirically or is best. only bool_results is worst. numerical checking is good.


        # ##############################################
        # # Compute Evaluator Extraction Rewards and Add to Replay Buffer
        # ##############################################
        # assert len(evaluator_extract_prompts) == len(evaluator_extract_responses), "generate evaluator extraction objects number must be equal to all inputs number"

        # if len(evaluator_extract_prompts) > 0:
        #     async with Timer("Compute evaluator extraction rewards"):
        #         eval_extract_prompts, eval_extract_responses, eval_extract_reward_tensors = await self.custom_evaluator_extract_reward_fn(
        #             evaluator_extract_prompts,
        #             evaluator_extract_responses,
        #             evaluator_extract_num_valid_options  # this variable should be populated in your extraction loop
        #         )

        #     # Only continue if eval_extract_reward_tensors are not all the same
        #     if eval_extract_reward_tensors is not None and len(eval_extract_reward_tensors) > 0:
        #         last_token_rewards = [score_tensor[-1].item() for score_tensor in eval_extract_reward_tensors if score_tensor.numel() > 0]
        #         if last_token_rewards:
        #             all_evaluator_extract_rewards_zero_or_one = (np.all(np.array(last_token_rewards) == 0.0) or 
        #                                             np.all(np.array(last_token_rewards) == 1.0))
        #         else:
        #             all_evaluator_extract_rewards_zero_or_one = False
        #     else:
        #         all_evaluator_extract_rewards_zero_or_one = False

        #     if not all_evaluator_extract_rewards_zero_or_one:
        #         # Pack these samples into batch tensors.
        #         async with Timer("Packing evaluator extraction samples"):
        #             (
        #                 extract_seq,
        #                 extract_attn_mask,
        #                 extract_num_actions,
        #                 extract_packed_seq_lens,
        #                 extract_custom_rewards
        #             ) = self._convert_prompts_outputs_to_batch_tensors_packing(
        #                 eval_extract_prompts, eval_extract_responses, eval_extract_reward_tensors, self.cfg.packing_max_len
        #             )
        #             action_masks = None
                
        #         # Run inference and compute values/advantages with exp_type="evaluator_extract"
        #         async with Timer("Evaluator Extraction Inference and Calculate"):
        #             evaluator_extract_experiences = await self.inference_and_calculates(
        #                 extract_seq,
        #                 extract_attn_mask,
        #                 action_masks,  # action masks not needed here
        #                 extract_num_actions,
        #                 extract_packed_seq_lens,
        #                 extract_custom_rewards,
        #                 exp_type="evaluator_extract"  # This sets exp_type tensor to 0.5
        #             )

        #         async with Timer("Calculate advantages and returns for evaluator experiences"):
        #             adv_tasks = []
        #             for experience in evaluator_extract_experiences:  # or evaluator_check_experiences
        ...
        #                 # (Optionally log metrics if needed.)
        #                 # Append the processed experience to the replay buffer.
        #                 self.replay_buffer.append(experience)
            

        # ##############################################
        # # Compute Evaluator Check Rewards and Add to Replay Buffer
        # ##############################################
        # assert len(evaluator_check_prompts) == len(evaluator_check_responses), "generate evaluator check objects number must be equal to all inputs number"

        # if len(evaluator_check_prompts) > 0:
        #     async with Timer("Compute evaluator check rewards"):
        #         # custom_evaluator_reward_fn expects a list of evaluator check prompts and responses.
        #         eval_check_prompts, eval_check_responses, eval_check_reward_tensors = await self.custom_evaluator_reward_fn(
        #             evaluator_check_prompts,
        #             evaluator_check_responses
        #         )

        #     # Only continue if eval_check_reward_tensors are not all the same
        #     if eval_check_reward_tensors is not None and len(eval_check_reward_tensors) > 0:
        #         last_token_rewards = [score_tensor[-1].item() for score_tensor in eval_check_reward_tensors if score_tensor.numel() > 0]
        #         if last_token_rewards:
        #             all_evaluator_check_rewards_zero_or_one = (np.all(np.array(last_token_rewards) == 0.0) or 
        #                                             np.all(np.array(last_token_rewards) == 1.0))
        #         else:
        #             all_evaluator_check_rewards_zero_or_one = False
        #     else:
        #         all_evaluator_check_rewards_zero_or_one = False

        #     if not all_evaluator_check_rewards_zero_or_one:
        #         async with Timer("Packing evaluator check samples"):
        #             (
        #                 check_seq,
        #                 check_attn_mask,
        #                 check_num_actions,
        #                 check_packed_seq_lens,
        #                 check_custom_rewards
        #             ) = self._convert_prompts_outputs_to_batch_tensors_packing(
        #                 eval_check_prompts, eval_check_responses, eval_check_reward_tensors, self.cfg.packing_max_len
        #             )
        #             action_masks = None

        #         async with Timer("Evaluator Check Inference and Calculate"):
        #             evaluator_check_experiences = await self.inference_and_calculates(
        #                 check_seq,
        #                 check_attn_mask,
        #                 action_masks,
        #                 check_num_actions,
        #                 check_packed_seq_lens,
        #                 check_custom_rewards,
        #                 exp_type="evaluator_check"
        #             )

        #         async with Timer("Calculate advantages and returns for evaluator check experiences"):
        #             adv_tasks = []
        #             for experience in evaluator_check_experiences:
        #                 adv_tasks.append(self._calc_advantages_and_returns(experience))
        ...
        if self.self_play:
            ...
            for idx in range(num_inspect):
                ...
                logger.debug(f"\n[STEP {self.global_step}] Query {idx+1} Query Text: {str(query_text['response'])}")
                ...
                logger.debug(f"[STEP {self.global_step}] Query {idx+1} Query Preference Reward: {preference_rewards[idx]}")
                logger.debug(f"[STEP {self.global_step}] Query {idx+1} Query LM Eval Reward: {lm_eval_rewards_list[idx]}")
                logger.debug(f"[STEP {self.global_step}] Query {idx+1} Query Diversity Reward: {diversity_rewards[idx]}")
                logger.debug(f"[STEP {self.global_step}] Query {idx+1} Query Logical Reward: {logical_rewards[idx]}")
    ...
    async def inference_and_calculates(
        ...
        for i in range(len(action_log_probs)):
            ...
            if exp_type == "evaluator_extract":
                exp_type_tensor = torch.tensor([0.6])
            if exp_type == "evaluator_check":
