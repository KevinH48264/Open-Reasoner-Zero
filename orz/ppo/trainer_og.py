    ...
    async def make_experience(self, all_inputs: Union[Tuple[str, dict], List[Tuple[str, dict]]], **generate_kwargs):
        ...
        if self.cfg.use_compute_reward_fn:
            async with Timer("Calculate custom rewards"):
                ...
                all_prompts, outputs, custom_rewards = await reward_fn(all_prompts, outputs, all_extras) # returns res_prompts (all_prompts[i]: List: [str]), res_responses (outputs["response"]: List[str]), res_score_tensors (List[Tensor] where len(Tensor([0, 0, 0, r])) = len(tokenized(outputs["response"]))) and r = score/reward for the response
