...
class PPOExpConfig(BasePPOExpConfig):
    ...
    # configs
    version = "v1249" # TODO: update this!
...
class CustomRewardTrainer(RayPPOTrainer):
    ...
    async def custom_reward_fn(
        ...
        for idx in range(len(outputs)):
            ...
            if output["final_answer"] == "":
                ...
                if stop_reason == "stop":
                    ...
                    # score = 0.0 # DEBUG: frozen policy for now
                else:
        ...
        self.writer.add_scalar("policy/avg_score_before_evaluator", avg_score, self.global_step)
    ...
    async def custom_query_reward_fn(
        ...
        if valid_indices:
            ...
            common_context = query_prompts[0]
            logical_prompt = (
                "You are GPT-4o. Your task is to evaluate the logical correctness of the following Query Assistant responses' target values based solely on their corresponding solutions and prompts, specifically task generation success criteria #5. "
                "For each response, decide if the proposed solution to the prompt is a full and complete proof and that the target expected answer is uncontentiously the one and only correct answer to the prompt. "
...
{current_query_assistant_responses_str}'''
        ...
        for idx, (prompt, output, extra) in enumerate(zip(query_prompts, query_outputs, query_output_extras)):
            ...
            if not valid:
                computed_reward = -1.0 #0.0 #-0.5 #-1.0 #0.5 #0.0 #-1.0 #0.0 #-0.5 # - 1.0 is the default value for invalid prompts to incentivize formatted valid prompts! and also for normalization to not penalize formatted valid but bad prompts --> format instability
            else:
                num_valid_queries += 1
