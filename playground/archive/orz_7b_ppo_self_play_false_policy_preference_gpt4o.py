...
class PPOExpConfig(BasePPOExpConfig):
    ...
    # configs
    version = "v1293" # TODO: update this!
...
#         # Launch GPT-4o calls in parallel.
#         tasks = [evaluate_policy_pair.remote(data) for data in pair_data_list]
#         pair_outputs = await asyncio.gather(*[asyncio.to_thread(ray.get, task) for task in tasks])

#         # Process evaluator outputs.
#         for ((idx1, idx2), eval_response, log_entry) in zip(pair_indices_list, pair_outputs, pair_log_entries):
#             log_entry["evaluator_response"] = eval_response
#             if eval_response:
#                 match = re.search(r'<answer>\s*([-]?1|[01])\s*</answer>', eval_response, re.DOTALL)
#                 if match:
...
#                     else:
#                         logger.error("Unexpected decision value; defaulting to 0.5")
#                         pairwise_rewards[idx1] = 0.5
#                         pairwise_rewards[idx2] = 0.5
#                         log_entry["decision"] = "unexpected; default 0.5"
#                 else:
#                     logger.error("Could not parse evaluator response; defaulting to 0.5")
#                     pairwise_rewards[idx1] = 0.5
#                     pairwise_rewards[idx2] = 0.5
#                     log_entry["decision"] = "parse error; default 0.5"
#             else:
#                 logger.error("No evaluator response; defaulting to 0.5")
#                 pairwise_rewards[idx1] = 0.5
#                 pairwise_rewards[idx2] = 0.5
...
#         self.writer.add_scalar("policy/equally_correct_ratio", equal_ratio, self.global_step)
...
#                     all_pair_log += f"# {key}:\n{value}\n\n\n"
#             all_pair_log += "\n----------------------------------------------------------------------\n"
#         self.writer.add_text("policy/all_criteria_evaluator_output", all_pair_log, self.global_step)
...
'''
    ...
    async def generate_vllm(
        ...
        **kwargs,
        ...
        # 判断对错
        global executor
        async def gpt4o_compare(prompt: str, expected: str, final: str) -> bool:
            url = "https://skilled-chigger-mentally.ngrok-free.app/api/send_request"
            evaluator_prompt = f"""You are a Policy Assistant Evaluator. Your task is to evaluate two answers (expected vs model) and decide whether they are the same. Assume that the expected answer is the correct answer to the original prompt, and that we're trying to figure out if the model answer is correct by seeing if it matches the expected answer.


POLICY ASSISTANT PROMPT: "{prompt}"
...
Please show your step-by-step reasoning between <think> and </think> tags, then provide your final decision between <answer> and </answer> tags as a single word: "yes" if the answers are the same, or "no" if they are not.
"""
            data = {
                "temperature": 0.0,
                "max_tokens": 4096,
            ...
            for attempt in range(max_retries):
                try:
                    ...
                    # Log the full prompt and raw response using self.writer if in "policy" mode.
                    if exp_type == "policy":
                        policy_evaluator_log = f"POLICY EVALUATOR PROMPT:\n{evaluator_prompt}\n\nPOLICY EVALUATOR RESPONSE: {result}"
                        self.writer.add_text("policy_evaluator", policy_evaluator_log, self.global_step)
                        logger.info(f"[STEP {self.global_step}] POLICY EVALUATOR LOG:\n\n{policy_evaluator_log}")

                    # Extract the answer using regex.
                    match = re.search(r'<answer>\s*(.*?)\s*</answer>', result, re.DOTALL)
                    if match:
                    ...
                    logger.error(f"GPT-4o comparison attempt {attempt+1} failed: {e}")
                    if attempt < max_retries - 1:
                        await asyncio.sleep(2)  # wait before retrying
            return False
