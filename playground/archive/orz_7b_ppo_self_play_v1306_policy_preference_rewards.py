...
class PPOExpConfig(BasePPOExpConfig):
    ...
    # configs
    version = "v1306" # TODO: update this!
    pretrain: Optional[str] = "Qwen/Qwen2.5-7B" #"EleutherAI/pythia-14m" # MINI-DEBUG "Qwen/Qwen2.5-7B" # TODO: or put your downloaded model path here!
    self_play = False
    ...
    # prompt_max_len: int = 1024 # MINI-DEBUG
...
class CustomRewardTrainer(RayPPOTrainer):
    ...
    async def custom_reward_fn(
        ...
        # Above is old code based on iscorrect based on the expected answer, but starting v1292 where math questions might not have the correct verifiable expected answer, we update the scores to be preference based on pairs. # NEW POLICY REWARD based on preferences
        # --- NEW: Update scores with pairwise GPT-4o evaluation ---
        # This block always runs (custom_reward_fn is only used in policy mode).
        # For each well-formatted output (score >= 0.0) we update its score using GPT-4o pairwise evaluation given expected answer might not always be correct, esp in self-play
        # If an output is ill-formatted (score == -0.5), its score is left unchanged.
        policy_evaluator_method = "qwen" #"gpt4o" #"qwen" # gpt4o

        # Group output indices by prompt.
        grouped_indices = defaultdict(list)
        for i, prompt in enumerate(prompts):
            grouped_indices[prompt].append(i)

        # Prepare lists for pairwise evaluation.
        pair_data_list = []
        pair_indices_list = []
        pair_log_entries = []
        # Initialize an array for the pairwise rewards.
        pairwise_rewards = [None] * len(outputs)

        # For each prompt group, form pairs.
        for prompt, indices in grouped_indices.items():
            if len(indices) < 2:
                # For a single response in a prompt, assign default reward 0.5.
                for idx in indices:
                    pairwise_rewards[idx] = 0.5
                    pair_log_entries.append({
                        "prompt": prompt,
                        "pair_indices": idx,
                        "pair_prompt": "N/A (only one response)",
                        "evaluator_response": "N/A",
                        "decision": "default 0.5",
                        "expected_answer": extras[idx].get("answer", "N/A"),
                        "response_0_match_expected": outputs[idx]["iscorrect"],
                    })
            else:
                ...
                for j in range(0, len(indices) - 1, 2):
                    idx1 = indices[j]
                    idx2 = indices[j + 1]
                    # Use the final_answer field from each output.
                    resp1 = "<think>" + outputs[idx1]["response"]
                    resp2 = "<think>" + outputs[idx2]["response"]
...
2) have a clear and concise final answer - while the thinking and reasoning can be long and exploratory, the final answer should directly answer the Policy Assistant Prompt and leave no room for misinterpretation, allowing the user with the ground truth answer to easily check the proposed final answer against the ground truth answer

3) have an as correct as possible final answer - the final answer to the problem must be as correct as possible, and as de-risked of being the wrong final answer as much as possible

4) thoughtful - the final answer should be supported by deep thinking and reasoning that is careful, detailed, evidence-based, and objective as possible to mitigate the biggest risks - being maximally truth-seeking and exploratory to achieve the most correct final answer

5) engaging - the reasoning should reflect a long-term problem solving and discovery growth mindset by engaging deeply with underlying concepts. This includes experimenting with new approaches to seek more evidence, seeking to understand why some methods and answers work and others don't, and exploring the problem space in a way that is not just rote memorization

6) novel - the thinking should explore new observations or alternative approaches that demonstrate opportunities for potential deeper understanding

7) formatted - the final answer should be between <answer> tags
...
Show your work in <think> </think> tags. And return the final answer in <answer> </answer> tags, for example <answer> 0 </answer>, <answer> 1 </answer>, or <answer> -1 </answer>. Think step by step inside <think> tags.
Policy Assistant Evaluator Response: <think>'''
        ...
        # GPT-4o Policy Evaluator
        @ray.remote(num_cpus=1)
        def evaluate_policy_pair_gpt4o(data):
            # LEARNING: DON'T USE BECAUSE UNSTABLE AND CAN FREEZE TRAINING
        ...
        # 2. In your pair evaluation block (inside custom_reward_fn), replace the old launch code with the following:
        # Launch Qwen-2.5 and GPT-4o evaluator calls in parallel.
        from vllm import SamplingParams
        sampling_params = SamplingParams(
            temperature=0.0, #self.cfg.temperature, #0.0, #self.cfg.temperature,
        ...
        policy_evaluator_prompts = [
            f"{pair_data['messages'][0]['content'][0]['text']}" for pair_data in pair_data_list
        ]
        qwen_results, stop_reasons = await gen_func(
            prompts=policy_evaluator_prompts, sampling_params=sampling_params, use_tqdm=False, truncate_prompt=True
        )
        gpt4o_tasks = [evaluate_policy_pair_gpt4o.remote(data) for data in pair_data_list]
        gpt4o_results = await asyncio.gather(*[asyncio.to_thread(ray.get, task) for task in gpt4o_tasks])

        # Initialize counters for our new metrics.
        total_pairs = len(pair_indices_list)
        total_response_length = 0
        parse_error_count = 0 # format error
        win_count = 0 # able to differentiate 0 or 1 preference
        unexpected_count = 0 # unexpected response after correct format (not 0, 1, or -1)
        un_response_count = 0 # no response
        qwen_gpt4o_match_count = 0 # see if both evaluators gave the same rating or not
        processed_pairs = 0

        for ((idx1, idx2), qwen_response, gpt4o_response, log_entry) in zip(pair_indices_list, qwen_results, gpt4o_results, pair_log_entries):
            log_entry["qwen_evaluator_response"] = qwen_response
            log_entry["gpt4o_evaluator_response"] = gpt4o_response

            # Track Qwen response length.
            tokens = self._tokenize([qwen_response], self.cfg.generate_max_len, padding=False)["input_ids"]
            response_length = len(tokens[0])
            total_response_length += response_length

            # Parse Qwen evaluator response.
            match = re.search(r'<answer>\s*(-1|0|1)\s*</answer>', qwen_response, re.DOTALL)
            if not qwen_response:
                un_response_count += 1
                decision_qwen = None
                log_entry["decision"] = "no response; default 0.5"
            elif not match:
                parse_error_count += 1
                decision_qwen = None
                log_entry["decision"] = "parse error; default 0.5"
            else:
                decision_qwen = match.group(1).strip()
                # Count clear wins (0 or 1).
                if decision_qwen in ["0", "1"]:
                    win_count += 1
                elif decision_qwen not in ["-1"]:
                    unexpected_count += 1
                    log_entry["decision"] = "unexpected; default 0.5"
            
            # Parse GPT-4o evaluator response in a similar manner.
            match_gpt4o = re.search(r'<answer>\s*(-1|0|1)\s*</answer>', gpt4o_response, re.DOTALL)
            if match_gpt4o:
                decision_gpt4o = match_gpt4o.group(1).strip()
            else:
                decision_gpt4o = None

            # Compare decisions if both are valid.
            if decision_qwen is not None and decision_gpt4o is not None:
                if decision_qwen == decision_gpt4o:
                    qwen_gpt4o_match_count += 1

            # Set the pairwise rewards as before using the Qwen decision.
            if policy_evaluator_method == "qwen":
                selected_decision = decision_qwen
            elif policy_evaluator_method == "gpt4o":
                selected_decision = decision_gpt4o

            if selected_decision == "0":
                pairwise_rewards[idx1] = 1.0
                pairwise_rewards[idx2] = 0.0
                log_entry["decision"] = "0 (Response 0 wins)"
            elif selected_decision == "1":
                pairwise_rewards[idx1] = 0.0
                pairwise_rewards[idx2] = 1.0
                log_entry["decision"] = "1 (Response 1 wins)"
            elif selected_decision == "-1":
                pairwise_rewards[idx1] = 0.5
                pairwise_rewards[idx2] = 0.5
                log_entry["decision"] = "-1 (Equal correctness)"
            else:
                pairwise_rewards[idx1] = 0.5
                pairwise_rewards[idx2] = 0.5
        ...
        # Compute metrics.
        avg_response_length = total_response_length / processed_pairs if processed_pairs > 0 else 0
        parse_error_percentage = (parse_error_count / processed_pairs * 100) if processed_pairs > 0 else 0
        win_percentage = (win_count / processed_pairs * 100) if processed_pairs > 0 else 0
        unexpected_percentage = (unexpected_count / processed_pairs * 100) if processed_pairs > 0 else 0
        un_response_percentage = (un_response_count / processed_pairs * 100) if processed_pairs > 0 else 0
        qwen_gpt4o_match_percentage = (qwen_gpt4o_match_count / processed_pairs * 100) if processed_pairs > 0 else 0

        # Log the metrics to TensorBoard using the required prefix.
        ...
        self.writer.add_scalar("policy_evaluator/qwen_gpt4o_match_percentage", qwen_gpt4o_match_percentage, self.global_step)
        logger.info(f"[STEP {self.global_step}] Policy evaluator metrics: avg_response_length: {avg_response_length:.2f}, parse_error%: {parse_error_percentage:.2f}, win%: {win_percentage:.2f}, unexpected%: {unexpected_percentage:.2f}, un_response%: {un_response_percentage:.2f}, Qwen-GPT4o match%: {qwen_gpt4o_match_percentage:.2f}")

        # Count how many pair evaluations resulted in equal correctness.
        num_equal = sum(1 for entry in pair_log_entries if "decision" in entry and entry["decision"].startswith("-1"))
        num_pairs = len(pair_indices_list)
        equal_ratio = num_equal / num_pairs if num_pairs > 0 else 0.0
        self.writer.add_scalar("policy_evaluator/equally_correct_ratio", equal_ratio, self.global_step)
        logger.info(f"[STEP {self.global_step}] Equal correctness ratio: {equal_ratio:.4f}")
        ...
        for entry in pair_log_entries:
            for key, value in entry.items():
                if key not in ["prompt"]:
                    all_pair_log += f"# {key}:\n{value}\n\n\n"
            all_pair_log += "\n----------------------------------------------------------------------\n"
        self.writer.add_text("policy_evaluator/all_criteria_evaluator_output", all_pair_log, self.global_step)
        logger.info(f"[STEP {self.global_step}] Policy pair evaluation log:\n{all_pair_log}")

        # Update the original scores for outputs that are well formatted.
        # If scores[i] is -0.5, leave it unchanged.
        for i in range(len(scores)):
            if scores[i] >= 0.0:
                scores[i] = pairwise_rewards[i]
        ### END OF NEW POLICY REWARD METHOD ###
