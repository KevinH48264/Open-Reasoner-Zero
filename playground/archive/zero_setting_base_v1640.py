...
"""
        ...
        if remove_half_GT_answers_from_train_dataset:
            # if "num_correct" not in dialogue[1]:
            #     if random.random() < 0.5: # random 50/50
            #         extra = {
            #             "prompt": prompt,
            #             "answer": "[NO GT ANSWER]",
            #             "target": dialogue[1]["ground_truth"]["value"],
            ...
            if len(dialogue) > 2 and 'pass_rate_72b_tir' in dialogue[2]:
                if dialogue[2]['pass_rate_72b_tir'] == "n/a" or float(dialogue[2]['pass_rate_72b_tir']) <= 0.625: # No GT for the hardest 50% of questions in openmathreasoning
                    extra = {
                        "prompt": prompt,
                        "answer": "[NO GT ANSWER]",
                        "target": dialogue[1]["ground_truth"]["value"],
            ...
            # elif len(dialogue) > 2 and 'difficulty' in dialogue[2]:
            #     if dialogue[2]['difficulty'] >= 6: # hardest 50% in deepmath
            #         extra = {
            #             "prompt": prompt,
            #             "answer": "[NO GT ANSWER]",
            #             "target": dialogue[1]["ground_truth"]["value"],
            #         } # 50% chance to remove the GT answer for training dataset
            # elif "num_correct" in dialogue[1]:
            #     # remove based on num_correct.
            #     if dialogue[1]["num_correct"] in [-1, 0, 1]:
            #         extra = {
            #             "prompt": prompt,
            #             "answer": "[NO GT ANSWER]",
            #             "target": dialogue[1]["ground_truth"]["value"],
            #         } # for 2048, remove when num_correct=0 = 50% of the dataset. for full 57K set, [0, 1] is 50% of the hardest questions
