...
RESPONSES_DIR = "v1442_eval_responses_t_1_on_train"
os.makedirs(RESPONSES_DIR, exist_ok=True)

# Evaluation dataset files
# Note: We add the new file to the list.
EVAL_FILES = [
...
##############################################
def load_eval_dataset(file_paths, tokenizer, prompt_max_len, goal, use_custom_dataset=False):
    """
    Loads dialogues from each file and creates the dataset.
    
    Parameters:
      - file_paths: list of paths for the evaluation files.
      - tokenizer: the tokenizer to use.
      - prompt_max_len: max length of the prompt.
      - goal: dataset goal (e.g. "math" or "countdown").
      - use_custom_dataset: if True, use CustomDataset even if goal != "countdown".
    """
    dialogues = []
    for file_path in file_paths:
        with open(file_path, "r", encoding="utf-8") as f:
            ...
            if goal == "countdown":
                ...
                # Attach file information to each item, if needed.
                for item in data:
...
def main():
    ...
    # Create a single vLLM instance to use for all files
    ...
    for file in EVAL_FILES:
        ...
        # Determine if this file should use CustomDataset
        use_custom = True if os.path.basename(file) == "orz_math_57k_collected.json" else False
        
        # Load the evaluation dataset for this file with the flag
        eval_dataset = load_eval_dataset([file], tokenizer, PROMPT_MAX_LEN, GOAL, use_custom_dataset=use_custom)
