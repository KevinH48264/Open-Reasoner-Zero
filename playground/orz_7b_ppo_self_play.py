...
class PPOExpConfig(BasePPOExpConfig):
    ...
    # configs
    version = "v1445" # TODO: update this!
    ...
    remove_half_GT_answers_from_train_dataset = False #True # if True, will remove half of the ground truth answers from the training dataset to encourage the model to learn from the policy evaluator's scores instead of just copying the GT answers. Set to False to keep all GT answers for training.
    ...
    policy_evaluator_balance_gt_iscorrect_experiences = False # NECESSARY: best practice to not have the policy evaluator overfit to yes/no given we're training against policy performance that starts off "no" 100% -> "yes" 100%. Ideally it's always 50/50 "no" and "yes"
...
Math Teacher Response: <think>'''
        ...
        self.writer.add_scalar("experiences/policy_experiences_percent_used", len(res_score_tensors) / res_score_tensors_len_before_balancing * 100 if len(prompts) > 0 else 0.0, self.global_step)
        self.writer.add_scalar("experiences/evaluator_experiences_percent_used", len(evaluator_res_score_tensors) / evaluator_res_score_tensors_len_before_balancing * 100 if len(evaluator_res_score_tensors) > 0 else 0.0, self.global_step)
